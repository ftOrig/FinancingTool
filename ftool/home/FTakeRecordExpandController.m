...
//    tableview.tableFooterView = [UIView new];
    ...
    UIButton *saveBtn = [AJCornerCircle buttonWithFrame:RECT(leading, tableview.maxY+15, btnW, 35) backgroundColor:baseNavColor title:@"保存" titleColor:AJWhiteColor titleFont:15 target:self action:@selector(saveBtnClick:) superview:self.view];
    // 再来
    UIButton *oneMoreBtn = [AJCornerCircle buttonWithFrame:RECT(saveBtn.maxX + 40, saveBtn.y, btnW, 35) backgroundColor:AJWhiteColor title:@"再来一笔" titleColor:baseNavColor titleFont:15 target:self action:@selector(oneMoreBtnClick:) superview:self.view];
    oneMoreBtn.layer.borderColor = baseNavColor.CGColor;
