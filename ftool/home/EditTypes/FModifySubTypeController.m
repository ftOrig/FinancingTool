...
@interface FModifySubTypeController ()
...
@implementation FModifySubTypeController
...
- (void)initView {
    ...
    NavBar *bar = [[NavBar alloc] initWithTitle:@"修改子分类" leftName:nil rightName:@"完成" delegate:self];
    
    UIView *bugetView = [UIView viewWithFrame:RECT(0, bar.maxY+15, MSWIDTH, 70) backgroundColor:AJWhiteColor superview:self.view];
    CGFloat imgW = 40.f;
    UIImageView *selectimageView = [UIImageView imageViewWithFrame:RECT(15, 15, imgW, imgW) imageFile:self.subType.iconName superview:bugetView];
    self.iconName = self.subType.iconName;
...
- (void)nextItemClick{
    ...
    if (self.nameF.text.length < 2) {
        ...
        if ([self.subType.name isEqualToString:self.nameF.text] && [self.subType.iconName isEqualToString:self.iconName]) {//未修改
            return;
        }
