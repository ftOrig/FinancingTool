...
-(void)setUpPickView{
	...
	CGFloat toolViewW = [UIScreen mainScreen].bounds.size.width;
    pickView.frame=CGRectMake(0, ZHToobarHeight, toolViewW, 162);
...
-(UIToolbar *)setToolbarStyle{
	...
	leftItem.tintColor = [UIColor ys_blue];
...
//    UIBarButtonItem *rightSpace=[[UIBarButtonItem alloc] initWithBarButtonSystemItem:UIBarButtonSystemItemFixedSpace target:self action:nil];
	...
	rightItem.tintColor = [UIColor ys_blue];
    ...
    toolbar.items=@[leftSpace,leftItem, centerSpace, centerSpace ,centerSpace,rightItem,leftSpace];
    return toolbar;
}
...
-(void)show{
    ...
    UIControl *bgView = [UIControl viewWithFrame:[UIScreen mainScreen].bounds backgroundColor:[UIColor blackColor] superview:[UIApplication sharedApplication].keyWindow];
    bgView.alpha = .5f;
    [bgView addTarget:self action:@selector(CancelClick) forControlEvents:UIControlEventTouchUpInside];
...
-(void)doneClick
{
    ...
    if ([self.delegate respondsToSelector:@selector(toobarDonBtnHaveClick:resultString:selectedRow:)]) {
        [self.delegate toobarDonBtnHaveClick:self resultString:_resultString selectedRow:[_pickerView selectedRowInComponent:0]];
    }
    [self removeFromSuperview];
    [[UIApplication sharedApplication].keyWindow.subviews.lastObject removeFromSuperview];
}
