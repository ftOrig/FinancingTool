...
- (void)setAnimate:(NSNumber *)animate {
    ...
    if ([animate boolValue]) {
        ...
        self.timer = [NSTimer timerWithTimeInterval:1.0 target:self selector:@selector(incrementOffset) userInfo:nil repeats:YES];
...
- (void)drawRect:(CGRect)rect {
    ...
    if (self.progress >= 0) {
        ...
        [self drawProgress:context withFrame:self.progressInset!= nil ? CGRectInset(rect, inset, inset) : rect];
    }
}
...
- (void)drawProgress:(CGContextRef)context withFrame:(CGRect)frame {
    ...
    if(self.progress > 0){
        ...
        if ([self.flat boolValue]) {
            ...
            CGColorRef colorBegin = self.color.CGColor;
           
            NSArray *colors=nil;
            CGGradientRef gradient;
            if (self.colorend) {
                CGColorRef colorend = self.colorend.CGColor;
                colors = @[(__bridge id)colorBegin, (__bridge id)colorend];
                gradient = CGGradientCreateWithColors(colorSpace, (__bridge CFArrayRef) colors, locations);
                CGContextDrawLinearGradient(context, gradient, CGPointMake(0, insetRect.size.height / 2), CGPointMake(insetRect.size.width, insetRect.size.height/2), kCGGradientDrawsBeforeStartLocation);

            }else{
                colors = @[(__bridge id)[self.color lighterColor].CGColor, (__bridge id)[self.color darkerColor].CGColor];
                gradient = CGGradientCreateWithColors(colorSpace, (__bridge CFArrayRef) colors, locations);
                 CGContextDrawLinearGradient(context, gradient, CGPointMake(insetRect.size.width / 2, 0), CGPointMake(insetRect.size.width / 2, insetRect.size.height), kCGGradientDrawsBeforeStartLocation);
            }
            CGContextRestoreGState(context);
...
- (void)drawRightAlignedLabelInRect:(CGRect)rect {
    ...
    label.font = [UIFont fontWithName:@".PingFang-SC-Thin" size:12];
    ...
    if (rect.size.width > 40) {
        ...
        [label drawTextInRect:CGRectMake(rect.size.width + 6, rect.origin.y, 30, rect.size.height)];
    }
}
...
- (NSNumber *)showBackgroundInnerShadow {
    if (_showBackgroundInnerShadow == nil) {
        return @YES;
    }
